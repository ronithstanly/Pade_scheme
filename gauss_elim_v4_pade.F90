...
!        a(1,1) = 5.0d0; a(1,2) = 1.0d0; a(1,3) = 2.0d0;
!    	a(2,1) = -3.0d0;a(2,2) = 9.0d0; a(2,3) = 4.0d0;
!	    a(3,1) = 1.0d0; a(3,2) = 2.0d0; a(3,3) = -7.0d0;
         
!        b(1) = 10.0d0;
!        b(2) = -14.0d0;
!        b(3) = -33.0d0;

        a(1,1)=1.0d0; a(1,2)=2.0d0;! a(1,3)=2.0d0; a(1,4)=-h/2.0d0;
        ...
        DO i=2,n-1
            !IF (MOD(i,2).NE.0) THEN
                !a(i,i-1)=0.0d0; a(i,i)=7.0d0; a(i,i+1)=h; a(i,i+2)=16.0d0; a(i,i+3)=0.0d0; a(i,i+4)=7.0d0; a(i,i+5)=-h;
                !a(i,j-1)=7.0d0; a(i,j)=h; a(i,j+1)=16.0d0; a(i,j+2)=0.0d0; a(i,j+3)=7.0d0; a(i,j+4)=-h;
                a(i,j)=1.0d0; a(i,j+1)=4.0d0; a(i,j+2)=1.0d0; !a(i,j+3)=0.0d0; a(i,j+4)=7.0d0; a(i,j+5)=-h;
                !a(i,j)=0.0d0; a(i,j+1)=7.0d0; a(i,j+2)=h; a(i,j+3)=16.0d0; a(i,j+4)=0.0d0; a(i,j+5)=7.0d0; a(i,j+6)=-h;
                j = j+1
            !ELSE IF (MOD(i,2)==0) THEN
                !a(i,i-1)=-9.0d0; a(i,i)=-h; a(i,i+1)=0.0d0; a(i,i+2)=8.0d0*h; a(i,i+3)=9.0d0; a(i,i+4)=-h; a(i,i+5)=0.0d0;
                !a(i,j-1)=-9.0d0; a(i,j)=-h; a(i,j+1)=0.0d0; a(i,j+2)=8.0d0*h; a(i,j+3)=9.0d0; a(i,j+4)=-h;
...
!    WRITE(*,*) 'Mod of 4/2=', MOD(4,2)
    ...
    SUBROUTINE B_Matrix
        ...
        !B Matrix
        b(1) = (-5.0d0/2.0d0)*f(1) + 2.0d0*f(2) + 0.5d0*f(3)!3.0d0*(f(2)-f(1))
        !b(2) = 9.0d0*f(1) - 12.0d0*f(2) + 3.0d0*f(3)

        DO i=2, n-1
            !IF (MOD(i,2).NE.0) THEN ! Odd
                b(i) = 3.0d0*(f(i+1)-f(i-1))!15.0d0 * (f(i+1)-f(i-1))
            !ELSE IF (MOD(i,2)==0) THEN ! Even
            !    b(i) = 24.0d0 * (f(i-1)-2.0d0*f(i)+f(i+1))
            !END IF 
        END DO

        !b(n-1)  = 3.0d0* (f(n)-f(n-1))
        b(n)    = (5.0d0/2.0d0)*f(n) - 2.0d0*f(n-1) - 0.5d0*f(n-2)!-9.0d0*f(n) + 12.0d0*f(n-1) - 3.0d0*f(n-2)   
